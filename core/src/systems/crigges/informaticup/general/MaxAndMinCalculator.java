...
//				 System.out.println(d.doubleValue());
			...
			addDoublesToArrayMin(min, list, countMin, max);
			addDoublesToArrayMin(min, formattedInputWords.getInputNeurons(), countMin += list.length, max);
			addDoublesToArrayMin(min, formattedInputEnding.getInputNeurons(),
					countMin += formattedInputWords.getInputNeurons().length, max);
			addDoublesToArrayMin(min, formattedInputFolder.getInputNeurons(),
					countMin += formattedInputEnding.getInputNeurons().length, max);
		}

		for (int i = 0; i < min.length; i++) {
			min[i] = Math.sqrt(min[i] / dataSetAll.size());
		}
	...
	private static double[] addDoublesToArrayMin(double[] input, double[] array, int startIndex, double[] max) {
		for (double d : array) {
			input[startIndex] += Math.pow(d - max[startIndex], 2);
			startIndex++;
		}
		return input;
	}
